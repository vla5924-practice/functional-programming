...
;;;;;;;;;;;;;;;;; Вставка ;;;;;;;;;;;;;;;;;
(define (let*->let expr)
  (define (add-let remaining-bindings body)
    (if (null? remaining-bindings)
      body
      (list (make-let
        (list (car remaining-bindings))
        (add-let (cdr remaining-bindings) body)))))
  (car (add-let (let*-bindings expr) (let*-body expr))))
;;;;;;;;;;;;;;;;; Конец вставки ;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;; Вставка ;;;;;;;;;;;;;;;;;
(define let*-test-1 '(let* ([x1 1] [x1 2] [x2 (lambda (x) (+ x 1))]) (+ x1 x1 (x2 3)) (displayln (+ x1 x1 (x2 3)))))
; (meval let*-test-1)
; 8

(define let*-test-2 '(let* ([x1 1] [x2 (+ x1 1)]) (+ x1 x2)))
; (meval let*-test-2)
; 3
;;;;;;;;;;;;;;;;; Конец вставки ;;;;;;;;;;;;;;;;;
