;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
;; ЗАДАНИЕ №3 (тесты объявлены в конце файла)
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
...
; В функциях ниже exp имеет вид '(set! x exp1)
...
; exp может иметь вид либо '(define x exp1),
; либо '(define (f x y z) body) = (define f (lambda (x y z) body)).
...
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
(define (fix? exp) (tagged-list? exp 'fix))
(define (fix-name exp) (caadr exp))
(define (fix-parameters exp) (cdadr exp))
(define (fix-body exp) (cddr exp))
(define (make-fix name params body) (cons 'fix (cons (cons name params) body)))
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
...
(define (letrec-bound-variables expr) (mmap first (second expr)))
(define (letrec-values expr) (mmap second (second expr)))
...
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
        ((lambda? exp) (make-rec-procedure rec-name-placeholder (lambda-parameters exp) (lambda-body exp) env))
        ((letrec? exp) (m-eval (letrec->application exp) env))
        ((fix? exp) (make-rec-procedure (fix-name exp) (fix-parameters exp) (fix-body exp) env))
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
        ((application? exp)
...
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
        ((rec-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment 
           (make-frame
            (cons (rec-name procedure) (rec-parameters procedure))
            (cons procedure arguments)
           )
           (rec-environment procedure)
          )
         )
        )
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
(define (letrec->application expr)
  (define (transform list1 list2 pred result)
    (if (null? list1)
     result
     (transform (cdr list1) (cdr list2) pred (append result (list (pred (car list1) (car list2)))))
    )
  )
  (define (make-named-lambda name exp)
   (if (lambda? exp)
    (make-fix name (lambda-parameters exp) (lambda-body exp))
    exp
   )
  )
  (let (
    (names (letrec-bound-variables expr))
    (values (letrec-values expr))
    (body (letrec-body expr))
  )
  (make-application
   (make-lambda names body)
   (transform names values make-named-lambda '())
  ))
)
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
(define (make-rec-procedure name parameters body env)
  (list 'rec-procedure name parameters body env))
(define (rec-procedure? proc)
  (tagged-list? proc 'rec-procedure))
(define (rec-name proc) (cadr proc))
(define (rec-parameters proc) (caddr proc))
(define (rec-body proc) (cadddr proc))
(define (rec-environment proc) (car (cddddr proc)))
(define rec-name-placeholder '__rec_func_name__)
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
...
(define letrec-test
  '(letrec (
    [f1 888]
    [f2 (lambda (x) (+ x 2))]
    [fact (lambda (zzz) (if (= zzz 0) 1 (* zzz (fact (- zzz 1)))))]
  ) (+ (fact 5) (f2 2)))
)
; 124
;;;;;;;;;;;;;;;;; ВСТАВКА ;;;;;;;;;;;;;;;;;
